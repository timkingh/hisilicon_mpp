...
static __inline HI_U32 cmos_get_ISO(cmos_gains_ptr_t p_gains)
{
    ...
    p_gains->iso =  (((HI_U64)_again * _dgain *_isp_gain* 100) >> (p_gains->again_shift + p_gains->dgain_shift+p_gains->isp_dgain_shift));
    return p_gains->iso;
}
		...
		HI_U32 exposure_shift)
{
    ...
    if (_dgain >= 4) { _dgain = 4; }
    else if (_dgain >= 2) { _dgain = 2; }
    else{_dgain = 1;}
