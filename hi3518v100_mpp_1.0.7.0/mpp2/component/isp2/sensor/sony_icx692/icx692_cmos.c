...
static HI_S32 cmos_get_ae_default(AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    pstAeSnsDft->u32MaxAgainTarget = 2;
    ...
    pstAeSnsDft->u32MaxDgain = 0x3ff;  /* 36db / 0.035db = 1028 */
    pstAeSnsDft->u32MinDgain = 0;
    pstAeSnsDft->u32MaxDgainTarget = 0x3ff;
...
{
    HI_U16 u16FullLine = (gu32FullLinesStd * u8SlowFrameRate) >> 4;
    
    pstAeSnsDft->u32MaxIntTime = u16FullLine - 3;

    sensor_write_register(0x407e, u16FullLine - 4);
...
static HI_U32 gau32DgainTbl[1023]=
{
...
static HI_U32 cmos_db_tbl(HI_U32 u32Db)
{
    HI_S32 i;
    HI_U32 u32Tmp, u32Step = 0;
    
    u32Tmp = 1024;
    for (i=0; i<u32Db; i++)
    {
        u32Tmp = (u32Tmp << 12) / 4080;
    }

    for (i = 0; i < 1023; i++)
    {
        if (u32Tmp <= gau32DgainTbl[i])
        {
            break;
        }
        u32Step++;
    ...
    if ((i >= 1) && ((u32Tmp - gau32DgainTbl[i-1]) < (gau32DgainTbl[i] - u32Tmp)))
    {
        u32Step--;
    }
